...
import {afterComplete, afterError, afterSuccess, beforeStart, isAction} from '../src/Helpers'
...
describe('redux easy action helper functions', () => {
    ...
    test('afterSuccess check if types match', () => {
        ...
        expect(afterSuccess({type: 'another-type-success'}, TestAction)).toBe(false)
    })
    ...
    test('afterComplete check if types match', () => {
        ...
        expect(afterComplete({type: 'another-type-complete'}, TestAction)).toBe(false)
    })
